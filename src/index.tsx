...
import { roundToPrecision, getColorForRating } from './utils';
...
export const RateStar = forwardRef<HTMLDivElement, StarRatingProps>((
  ...
  const componentId = useRef(`star-rating-${Math.random().toString(36).substring(2, 9)}`).current
  ...
  const displayRating = hoverRating > 0 ? hoverRating : keyboardFocusValue !== null ? keyboardFocusValue : rating
  ...
  return (
    ...
    >
      {[...Array(maxRating)].map((_, index) => {
        ...
        const isCurrentKeyboardFocus = keyboardFocusValue !== null && index === Math.max(0, Math.ceil(keyboardFocusValue) - 1)
        ...
        const starFillColor = activeColorsEnabled ? getColorForRating(displayRating, maxRating, activeColorsEnabled, customActiveColors) : undefined;

        if (variant === 'circle' || variant === 'square') {
          const activeColorForGradient = activeColorsEnabled
            ? getColorForRating(displayRating, maxRating, activeColorsEnabled, customActiveColors)
